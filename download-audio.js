...
import { findMissingAudio } from './find-missing-audio.js'
...
//
// Brian is saying a lof "uh" before words. Might work better with complete sentences.
...
try {
  const wordsList = findMissingAudio([...WORDS.round1, ...WORDS.round2, ...WORDS.round3])
  if (!wordsList?.length) {
    throw new Error(`No words found needing audio.`)
  }
